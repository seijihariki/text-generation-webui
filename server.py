...
from modules.lcagent import generate as lc_generate
...
def create_interface():
    ...
    with gr.Blocks(css=ui.css if not shared.is_chat() else ui.css + ui.chat_css, analytics_enabled=False, title=title) as shared.gradio['interface']:
        if shared.is_chat():
            ...
            def cai_langchain_wrapper(message, generate_state, name1, name2, context, chat_mode, end_of_turn):
                response, memory = lc_generate(message, generate_state)

                shared.history['visible'].append([message, response])
                yield chat_html_wrapper(shared.history['visible'], name1, name2, chat_mode)

            cai_chatbot_wrapper = chat.cai_chatbot_wrapper if not shared.args.agent else cai_langchain_wrapper
            regenerate_wrapper = chat.regenerate_wrapper

            gen_events.append(shared.gradio['Generate'].click(
                lambda x: (x, ''), shared.gradio['textbox'], [shared.gradio['Chat input'], shared.gradio['textbox']], show_progress=False).then(
                cai_chatbot_wrapper, shared.input_params, shared.gradio['display'], show_progress=shared.args.no_stream).then(
            ...
            gen_events.append(shared.gradio['Regenerate'].click(
                regenerate_wrapper, shared.input_params, shared.gradio['display'], show_progress=shared.args.no_stream).then(
            ...
            shared.input_params = [shared.gradio[k] for k in ['textbox', 'generate_state', 'stopping_strings']]
            output_params = [shared.gradio[k] for k in ['textbox', 'markdown', 'html']]
            gen_events.append(shared.gradio['Generate'].click(generate_reply_wrapper, shared.input_params, output_params, show_progress=shared.args.no_stream))
